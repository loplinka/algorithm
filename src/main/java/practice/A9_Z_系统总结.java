...
public class A9_Z_系统总结 {
     ...
     *
     * 背景?
     * 老的模型系统是一个第三方依赖的方式集成在其他系统中,这个系统本身出了规则计算还承担了其他诸如可带额度咨询、记账、机构资金池管理等职能,经过大促和业务的长期发展后,
     * 逐渐暴露出以下弊端:
     ...
     *      (2)每个规则包,大业务相同的使用一个Git工程,里边不同字业务的采用Module进行区分; 比如:淘系列就是一个git工程,套系的下天猫就是一个Module,淘宝是另一个Module,1688就是另一个Module
     * 4.版本管理:
     ...
     *                  第二种策略就是根据用户百分比,比如用户id 前1%的先切留,然后手动控制灰度开关,3%,10%等切换过去;
     *                  前一种灰度策略是根据时间推移自动的,后一种是手动的; 一般老业务升级选择自动的,新业务上线选择手动的;
     *
     * 挑战点或者难点?
     * A:如何保证规则包的动态发布要求?
